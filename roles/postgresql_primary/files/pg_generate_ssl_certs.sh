...
#
#   Author  : mgale
#   Purpose : Generate ssl certs for Postgres
#   Date    : Jan 2019
#
#   Example Run :   $ ./pg_generate_ssl_certs.sh `hostname`
#
FQDN=$1
...
## FORMAT FILES FOR POSTGRES
cp /var/certs/`hostname`/`hostname`.key /var/certs/pg_server.key
cp /var/certs/`hostname`/`hostname`.csr /var/certs/pg_server.csr
chmod 600 /var/certs/pg_server.key
chmod 755 /var/certs
chown -R postgres:postgres /var/certs/
